...
#include "AnimModifiers/AnimToolsFootstepModifier.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimToolsFootstepModifier)

#define LOCTEXT_NAMESPACE "FootstepAnimEventsModifierBase"

UAnimToolsFootstepModifier::UAnimToolsFootstepModifier() : Super()
{
	SampleRate = 60;
...
void UAnimToolsFootstepModifier::OnApply_Implementation(UAnimSequence* InAnimation)
{
	...
	// Process animation asset
	{
		...
		for (int SampleIndex = 0; SampleIndex < SampleNum; ++SampleIndex)
		{
			...
			{
				const FFootstepDefinition & FootDef = FootDefinitions[FootIndex];
		...
		for (int SampleIndex = 0; SampleIndex < (SampleNum - 1); ++SampleIndex)
		{
			...
			{
				const FFootstepDefinition & FootDef = FootDefinitions[CurrFootIdx];
				...
				if (SampleIndex > 1)
				{
					...
					if (FootDef.bShouldGenerateSyncMarkers && CanWePlaceEventAtSample(FootState, FootDef.SyncMarkerDetectionTechnique))
					{
						...
						{
							const float FinalSyncMarkerTime = FootDef.SyncMarkerDetectionTechnique == EFootstepDetectionTechnique::FootBoneSpeed ? FootState.TimeAtMinFootSpeedBelowThreshold : SampleTime;
					...
					if (FootDef.bShouldGenerateNotifies && CanWePlaceEventAtSample(FootState, FootDef.FootstepNotifyDetectionTechnique))
					{
						...
						{
							const float FinalAnimNotifyTime = FootDef.FootstepNotifyDetectionTechnique == EFootstepDetectionTechnique::FootBoneSpeed ? FootState.TimeAtMinFootSpeedBelowThreshold : SampleTime;
        
							// 1. Capture the newly spawned notify
							UAnimNotify* SpawnedNotify = UAnimationBlueprintLibrary::AddAnimationNotifyEvent(InAnimation, FootDef.FootstepNotifyTrackName, FinalAnimNotifyTime, FootDef.FootstepNotify);
        
							// 2. Cast it to our custom class and inject EVERYTHING!
							if (UAN_Footstep* CustomFootstepNotify = Cast<UAN_Footstep>(SpawnedNotify))
							{
								CustomFootstepNotify->FootBoneName = FootDef.FootBoneName;
            
								// If the user assigned a Data Asset in the modifier, pass it down to the notify
								if (FootDef.FootstepDataAsset)
								{
									CustomFootstepNotify->FootstepData = FootDef.FootstepDataAsset;
								}
							}
						}
						else
				...
				// Update foot state
				{
					if (SampleIndex > 1)
					{
						const bool bDidEventPlacementTestPassUsingFootBoneSpeed = CanWePlaceEventAtSample(FootState, EFootstepDetectionTechnique::FootBoneSpeed);
...
void UAnimToolsFootstepModifier::OnRevert_Implementation(UAnimSequence* InAnimation)
{
	...
	for (const FName ProcessedNotifyTrack : ProcessedNotifyTracks)
	{
		...
		{
			for (const FFootstepDefinition& FootDef : FootDefinitions)
			{
...
void UAnimToolsFootstepModifier::ValidateNotifyTracks(UAnimSequence* InAnimation)
{
...
void UAnimToolsFootstepModifier::GatherNotifyTracksInfo(const UAnimSequence* InAnimation)
{
	for (const FFootstepDefinition& FootDef : FootDefinitions)
	{
...
void UAnimToolsFootstepModifier::PrepareNotifyTrack(UAnimSequence* InAnimation, FName InRequestedNotifyTrackName)
{
...
bool UAnimToolsFootstepModifier::CanWePlaceEventAtSample(const FFootSampleState& InFootSampleState, EFootstepDetectionTechnique DetectionTechnique) const
{
	switch(DetectionTechnique)
	{
		case EFootstepDetectionTechnique::PassThroughReferenceBone: return InFootSampleState.RefBoneTranslationDotRefBoneToFootBoneVec > 0.0f && InFootSampleState.PrevRefBoneTranslationDotRefBoneToFootBoneVec < 0.0f;
		case EFootstepDetectionTechnique::FootBoneReachesGround: return !InFootSampleState.bWasFootBoneInGround && InFootSampleState.bIsFootBoneInGround;
		case EFootstepDetectionTechnique::FootBoneSpeed: return (InFootSampleState.PrevFootBoneSpeed < SpeedThreshold && InFootSampleState.FootBoneSpeed >= SpeedThreshold) || (InFootSampleState.bIsLast && InFootSampleState.FootBoneSpeed < SpeedThreshold && !FMath::IsNearlyEqual(InFootSampleState.MinFootSpeedBelowThreshold, MAX_FLT));
		default: return false;
	}
}

float UAnimToolsFootstepModifier::ComputeBoneSpeed(const FAnimPose& InPose, const FAnimPose& InFuturePose, float InDelta, FName InFootBoneName) const
{
